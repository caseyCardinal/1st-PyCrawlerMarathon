...
class JSONPipeline(object):
    def open_spider(self, spider):
        self.start_crawl_datetime = datetime.now().strftime('D%Y%m%d_T%H%M%S')
    ...
    def close_spider(self, spider):
        self.end_crawl_datetime = datetime.now().strftime('D%Y%m%d_T%H%M%S')
        ...
        if spider.name == 'PTTCrawler' and spider.board:
            self.store_file_path = self.dir_path / '{board}-{datetime}.json'.format(
                    board=spider.board, datetime=datetime.now().strftime('D%Y%m%d_T%H%M%S'))
